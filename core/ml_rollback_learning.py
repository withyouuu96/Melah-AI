...
def get_latest_learning_backup():
    files = glob.glob("models/backup/ml_learning_*.json")
    if not files:
        return None
...
def rollback_learning_history():
    ...
    current_path = 'models/ml_learning_history.json'
    if os.path.exists(current_path):
        backup_path = f"models/backup/ml_learning_{os.path.basename(latest_backup)}"
